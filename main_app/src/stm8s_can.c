...
// Globals!
uint8_t  l_can_data[10]={0};
uint8_t  l_can_receive_flag=0;
                                  ...
                                  uint8_t *CAN_Data)
{
    ...
    CAN_TxStatus = CAN_TransmitStatus(CAN_TransmitMailBox_0);
...
void CAN_Receive(void)
{
    ...
    if((_Id == 0x728) || (_Id == 0x7E8 ))       //  _Id == 0x728, 7E8 are the vehicle ID's
    {
        l_can_data[0] = _Data[0];
        l_can_data[1] = _Data[1];
        l_can_data[2] = _Data[2];
        l_can_data[3] = _Data[3];
        l_can_data[4] = _Data[4];
        l_can_data[5] = _Data[5];
        l_can_data[6] = _Data[6];
        l_can_data[7] = _Data[7];    
    }
    ...
    l_can_receive_flag = 1;
    if((_Id == 0x7DF) &&((Timeout_EOL_Flag==FALSE) ||(Tester_Present==TRUE)))
    {
        Tester_Present=TRUE;              
    }
    else
